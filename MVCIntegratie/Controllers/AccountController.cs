...
namespace MVCIntegratie.Controllers
{
   ...
   public partial class AccountController : Controller
   {
      ...
      [AllowAnonymous]
      [ValidateAntiForgeryToken]
      public virtual async Task<ActionResult> Register(RegisterViewModel model, string deelplatform)
      {
         if (ModelState.IsValid)
         {
            var user = new MyUser
            ...
            if (result.Succeeded)
            {
               await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false).ContinueWith(c =>
               {
                  Deelplatform platform = platformMng.GetDeelplatform(deelplatform);
                  Gebruiker gebruiker = platformMng.GetGebruiker(user.Id);
                  gebruiker.Deelplatformen.Add(platform);
      ...
      [AllowAnonymous]
      [ValidateAntiForgeryToken]
      public virtual async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl, string deelplatform)
      {
